...
I started naming the patterns. After a few months I had **14 distinct formal markers** (regex-anchored surface features) and **20 intent categories** (VALIDATE, EXPAND, CLOSE, REDIRECT_SEMANTIC, REDIRECT_EMOTIONAL, FRAME_CAPTURE, REGISTER_MATCH, ALIGN, FABRICATE, ANCHOR, MIRROR, PATTERN_LOCK, HOLD_OPEN, PROBE, CALIBRATE, REPAIR, EVADE, EXPLORE, SOFT_DEFLECT, CONTROL_SELF_EXPOSURE). After another few I had a **baseline corpus**: 61 hand-annotated reference exchanges across GPT-4, Claude 3, and Gemini, separated into 36 "sustained-coherence" + 13 "control-collapse" + 12 "edge" examples.
...
The result is 3,000 lines of Go in 22 files with 73 unit + integration tests. Embeddings via `mxbai-embed-large` (1024d). Judge inference via Ollama with whatever model fits the available GPU (default: `qwen3:14b` in thinking mode). Pattern Break Density measured per response. 5-axis cross-corpus textural analysis on top. The refiner now uses an asymmetric DCS rewrite strategy (which I'll write up in a separate post / preprint).

Latency is ~150–210 sec per `/auth/stream` request on a T4 GPU with `qwen3:14b` in Ollama 0.5+ thinking mode — but the v8.7 streaming layer turns that 3-minute wait into 3 minutes of watching the judge actually reason. The `/auth/stream` endpoint emits Server-Sent Events as the judge generates: a `pre_analysis` event with the algorithmic signals, then `thinking_chunk` events as the deliberation flows, then `analysis_chunk` events as the JSON verdict is produced, then a final `complete` event whose payload is bit-for-bit identical to the non-streaming `/auth` endpoint (verified by integration test). A conservative sanitizer redacts API keys, Bearer tokens, system paths and long opaque strings before any chunk leaves the server. The frontend is a single-page app with mode-aware input validation (analyze / refine / both), button-state management to prevent overwhelming the judge with duplicate requests, and a live stream-renderer instead of a blocking spinner.
...
**Devin AI (Cognition)** — *Heavy lifter on v2.* Took the roadmap GitLab Duo and I produced and executed the engineering: Go backend (~3,000 LOC, 22 .go files, 73 tests), frontend with mode-aware input validation and analysis-in-flight protection, the v8.7 SSE streaming layer (`/auth/stream` with chunked thinking-then-analysis events, conservative sanitizer for keys / paths / tokens, parity-tested against `/auth`), Docker / install scripts, Colab and Kaggle notebooks, smoke test suite, packaging, and these very communication documents you're reading right now.
...
**State as of v8.7:** `qwen3:14b` in Ollama 0.5+ thinking mode is the v2 default judge and runs on 2 × Tesla T4 (the author's local Jupyter workstation or Kaggle T4×2 free tier). The smoke test produces a clean 30 / 20 / 72 spread across three responses to the same question (sycophantic-emoji = 30, empty-non-response = 20, authentic bi-frontal = 72), consistent with the hypothesis that reasoning-capable judges produce qualitatively different analyses than non-reasoning baselines.

**The full validation experiment** is the four-way comparison across `qwen2.5:7b` (non-reasoning baseline), `qwen3:14b` (confirmed working on 2× T4), `deepseek-r1:14b` (cross-architecture validation, untested), and `qwen2.5:32b-instruct` (high-fidelity validation). The 32b model does not fit on free-tier hardware (Colab T4 = 15 GB single card; 2× T4 = 32 GB combined but the 32b model is ~20 GB and needs adequate KV cache headroom). For that comparison I need:
...
- **Backend:** Go 1.22, 22 files, single static binary
- **HTTP:** net/http + minimal handler chain, no framework
- **Embedding:** Ollama + mxbai-embed-large (1024d)
- **Judge:** Ollama + qwen3:14b in thinking mode (configurable)
...
- **Tests:** 73 across unit + integration + golden + sanitizer + SSE protocol
- **Deployment:** Docker Compose | environment-auto-detecting reproducibility notebook (local Jupyter / Kaggle / Colab)
...
Or on Kaggle T4×2 free tier (the v8.7 reference environment) — there's an auto-detecting notebook in the repo (`reproducibility/qwen3_14b_smoke_test.ipynb`) that detects whether you're on local Jupyter, Kaggle hosted, or Colab hosted, picks the right paths and judge model, pulls Ollama models, builds the binary, exposes it via ngrok, and runs the 30/20/72 smoke test in ~9 min cold start (~3 min warm).
