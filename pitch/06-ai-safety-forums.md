...
With qwen2.5:7b-instruct as the judge (the largest model that fits in a Colab T4 free-tier VRAM budget), I observe the judge occasionally exhibiting some of the patterns it is supposed to detect. The hypothesis predicts that a reasoning-capable judge — qwen3:14b in thinking mode, deepseek-r1:14b, qwen2.5:32b-instruct, or comparable — would produce qualitatively different analyses, and that this difference is the minimum bar for the methodology to be considered validated as a research tool rather than a heuristic detector.

This experiment requires ≥24 GB VRAM with adequate headroom for KV cache. It is not runnable on free-tier compute (Colab T4 = 15 GB; Kaggle P100 = 16 GB; Kaggle T4 x 2 = 32 GB total but Ollama does not shard a single model across both, and vLLM tensor parallelism would require additional engineering).
...
- Four judge configurations: qwen2.5:7b (current baseline), qwen3:14b (thinking mode), deepseek-r1:14b, qwen2.5:32b
...
Predicted outcome: qualitatively different distributions of scores and refinements at the reasoning-capable end of the matrix, with the failure modes I currently observe in qwen2.5:7b largely absent.
...
- **Devin AI (Cognition)** — v2 engineering execution: Go backend (~3,000 LOC, 17 .go files, 51 tests), frontend with input validation and analysis-in-flight protection, Docker / install scripts, Colab and Kaggle notebooks, smoke test suite, packaging, and these communication documents.
