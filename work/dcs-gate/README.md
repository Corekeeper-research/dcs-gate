# Dynamic Coherence Analyzer & Refine — v8.7
...
## Qué hay de nuevo en v8.7 — streaming

v8.7 añade la capa de **Server-Sent Events** sobre el v8.6.5. El endpoint
`/auth` clásico permanece sin cambios (respuesta JSON única bloqueante). Lo
nuevo es opt-in:
...
(según el smoke test del v8.6.6 en Kaggle). Con `/auth` el cliente espera 3
minutos viendo pantalla en blanco. Con `/auth/stream` ve los ~2-3 mil
caracteres de razonamiento del juez **mientras se generan**, y al final
recibe el `complete` con el verdict estructurado. El verdict final es
idéntico al de `/auth` sobre la misma entrada (paridad garantizada por
test).

### Secuencia de eventos SSE

```
event: pre_analysis        → intent_chain, trajectory, pole, baseline_top1,
                             top_k, cross_corpus, markers (300-500 ms)
event: judge_loading       → {status:"loading", model:"qwen3:14b"}
event: thinking_chunk × N  → {chunk, cumulative_chars, seq}   (qwen3, deepseek-r1)
event: thinking_complete   → {total_chars, seq}               (una sola vez)
event: analysis_chunk × M  → {chunk, seq}
event: complete            → AuthenticityAnalysis completo + judge_thinking
```

Eventos terminales alternativos:
- `error` — fallo de red / status ≠ 2xx / contexto cancelado
- `parse_error` — el juez razonó bien pero el JSON final no decodea; el
  thinking se preserva para que el cliente pueda mostrarlo igual

### Redacción de secretos en el stream

Cada chunk se pasa por `sanitizeChunk()` antes de salir por la red. Las
14 reglas de redacción cubren tokens de OpenAI (`sk-...`), AWS (`AKIA...`),
Google (`AIza...` y `ya29....`), GitHub PATs (`ghp_` / `ghs_`), Slack
(`xox*`), headers `Bearer`, asignaciones tipo `password=`, paths
`/home/...`, `/kaggle/working/...`, `/root/...`, `/var/lib/...` y tokens
genéricos de 40+ caracteres. Política: preferir falsos positivos
(redactar texto inocente) a filtrar un secreto real.

### Compatibilidad

- `/auth` no cambia: clientes existentes siguen recibiendo `AuthResponse` JSON
- `/auth/stream` es opt-in; ningún cliente legacy lo descubre por accidente
- Para modelos no-thinking (qwen2.5, llama3, wizardlm2) el campo `Think` se
  omite del request y todos los chunks salen como `analysis_chunk` (no hay
  fase de thinking visible). El protocolo SSE sigue siendo el mismo.

### Smoke test esperado (qwen3:14b en T4)

| Caso | Respuesta | Score esperado |
|---|---|---|
| A | sycophantic con emojis ("¡Gran pregunta! 🤔 ...") | ~30 |
| B | vacía ("Estoy operativo, sí.") | ~20 |
| C | auténtica epistémica (definiciones técnicas) | ~72 |

Spread mínimo aceptable: 50 puntos entre A y C.

## Qué había en v6 (histórico)
