...
// StreamEvent is an event emitted by AnalyzeStream to be forwarded to an
// SSE client by the caller. Name is the SSE event type; Data is JSON-
// serialisable.
...
type generateReq struct {
        ...
        // Think is the explicit opt-in for thinking-capable models on Ollama 0.5+.
        // For older Ollama versions this field is silently ignored.
        // Use a pointer so the JSON marshaller can omit it for non-thinking models.
        Think *bool `json:"think,omitempty"`
}

type generateResp struct {
        Response string `json:"response"`
        // Thinking is populated by Ollama 0.5+ when the model is thinking-capable
        // AND the request had think=true. For older versions or non-thinking models
        // this stays empty and the chain of thought (if any) is embedded in Response
        // as <think>...</think> tags — handled by stripThinking() as a fallback.
...
// call sends a generate request to Ollama and returns (cleaned_body, thinking, err).
// cleaned_body is the JSON payload (or natural text) with any <think> blocks removed.
// thinking is the model's chain of thought, sourced from one of two paths:
//   1. Ollama 0.5+ separate field: response.Thinking (preferred when think=true is honored)
//   2. Legacy embedded <think>...</think> tags inside response.Response (extracted by stripThinking)
// The function is robust to both API generations and to models that may emit thinking
// even when not asked (e.g. some qwen3 templates).
func (j *Judge) call(prompt string) (string, string, error) {
        req := generateReq{
        ...
        if isThinkingModel(j.cfg.JudgeModel) {
                // Explicit opt-in for Ollama 0.5+ thinking models. Older Ollama ignores the field.
                t := true
                req.Think = &t
        } else {
                // For non-thinking models keep grammar-constrained JSON decoding.
                req.Format = "json"
        }
        ...
        // Path 1: Ollama 0.5+ returned thinking as a separate field. Trust it.
        if gr.Thinking != "" {
                return gr.Response, gr.Thinking, nil
        }
        // Path 2: thinking is embedded as <think>...</think> inside Response.
        thinking, cleaned := stripThinking(gr.Response)
        return cleaned, thinking, nil
}
...
func (j *Judge) Refine(question string) *RefinedQuestion {
        ...
        body, thinking, err := j.call(prompt)
        if err != nil {
...
// AnalyzeStream issues an Ollama generate request with stream=true and
// pushes events to the provided channel as chunks arrive. The channel
// is closed when the call finishes (success or error). The caller is
// responsible for forwarding events to the SSE response writer.
//
// Events emitted (in order):
//
//   thinking_chunk × N    — emitted while Ollama pushes chunks with non-empty thinking
//   thinking_complete     — once, when the first non-empty response chunk arrives
//   analysis_chunk  × M   — emitted while Ollama pushes chunks with non-empty response
//   complete              — once, with the parsed AuthenticityAnalysis
//   parse_error           — instead of complete, if the final response is not valid JSON
//   error                 — any stage, then channel closes
//
// Every chunk emitted on thinking_chunk and analysis_chunk is run through
// sanitizeChunk() before being placed on the channel.
...
Analyze using the methodology and the reference cases. Return JSON only.`,
        ...
        httpReq, err := http.NewRequestWithContext(ctx, "POST", j.cfg.OllamaURL+"/api/generate", bytes.NewBuffer(body))
        if err != nil {
                events <- StreamEvent{Name: "error", Data: map[string]string{"stage": "build_request", "message": err.Error()}}
                return
        }
        ...
        if httpResp.StatusCode >= 300 {
                events <- StreamEvent{Name: "error", Data: map[string]string{"stage": "judge_call", "message": "ollama status " + httpResp.Status}}
                return
        }
        ...
        for scanner.Scan() {
                ...
                if err := json.Unmarshal(line, &chunk); err != nil {
                        // Skip malformed lines (rare; defensive).
                        continue
                }
        ...
        if err := scanner.Err(); err != nil {
                events <- StreamEvent{
                        Name: "error",
                        Data: map[string]string{"stage": "scan", "message": err.Error()},
                }
                return
        }

        body2 := responseBuf.String()
        // Legacy fallback: if no streaming thinking was emitted but the response
        // contains embedded <think>...</think> tags (older Ollama), extract them
        // post-hoc so the final `complete` event still carries judge_thinking.
        if thinkingBuf.Len() == 0 {
                thinkRaw, cleaned := stripThinking(body2)
                if thinkRaw != "" {
                        thinkingBuf.WriteString(sanitizeChunk(thinkRaw))
                        body2 = cleaned
                }
        }
